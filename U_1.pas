...
begin
  WITH FDQuery1 do
     begin
       close;
       sql.Clear;
       sql.Add('delete from principal where cpf = ('''+To_delete.Text+''')');
       //sql.Add(');
       ExecSql;
       To_delete.Text := '';
       msg_excl.Visible := true;
     end;
...
begin
    ...
    if msg_excl.Visible then
       msg_excl.Visible := false;
end;

procedure TForm1.BuscarClick(Sender: TObject);
begin
    grid_notas.Visible := True;
    FDQuery1.Open('select * from principal where nome = '+Src_name.Text);
    if msg_excl.Visible then
       msg_excl.Visible := false;
end;
